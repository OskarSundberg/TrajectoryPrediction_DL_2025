...
class SEASTAR(torch.nn.Module):
    ...
    def __init__(self, src_dims, dist_dims, type_dims, num_types, num_types_dist, hidden=256, num_layers=16, num_heads=8, src_len=10, tgt_len=40, dropout=0.1):
        ...
        self.embedding_layer = SEASTAREmbedding(src_dims, dist_dims, type_dims, num_types, num_types_dist, src_len)

        # Transformer encoder layers for spatial, temporal, and environmental features.
        self.temporal_encoder_layer = nn.TransformerEncoderLayer(d_model=self.embedding_size, nhead=num_heads, batch_first=True)
        self.spatial_encoder_layer = nn.TransformerEncoderLayer(d_model=self.embedding_size, nhead=num_heads, batch_first=True)
        self.environmental_encoder_layer = nn.TransformerEncoderLayer(d_model=self.embedding_size, nhead=num_heads, batch_first=True)
        
        # Stacked transformer encoders for spatial, temporal, and environmental processing.
        self.spatial_encoder_1 = nn.TransformerEncoder(self.spatial_encoder_layer, num_layers)
        self.spatial_encoder_2 = nn.TransformerEncoder(self.spatial_encoder_layer, num_layers)
        self.temporal_encoder_1 = nn.TransformerEncoder(self.temporal_encoder_layer, num_layers)
        self.temporal_encoder_2 = nn.TransformerEncoder(self.temporal_encoder_layer, num_layers)
        self.environmental_encoder_1 = nn.TransformerEncoder(self.environmental_encoder_layer, num_layers)
        self.environmental_encoder_2 = nn.TransformerEncoder(self.environmental_encoder_layer, num_layers)

        # Decoders for spatial, temporal, and environmental feature projection.
        self.decoder1 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
        self.decoder2 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
        self.decoder3 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
        self.decoder4 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
        self.decoder5 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
        self.decoder6 = MLPDecoder(self.embedding_size, self.hidden_size, self.output_size, dropout=dropout)
    ...
    def forward(self, src, dist, type, src_mask=None, dist_key_padding_mask=None):
        """
        ...
        src_temporal_embedded = self.embedding_layer(src, is_src=True)
        src_temporal_embedded = self.dropout(src_temporal_embedded)  # Apply dropout.
        src_temporal_embedded = self.relu(src_temporal_embedded)  # Apply ReLU activation.

        # Embed the distance sequence for spatial encoding.
        src_dist_embedded = self.embedding_layer(dist, is_src=False, src_tensor=src, type_tensor=type)
        src_dist_embedded = self.dropout1(src_dist_embedded)  # Apply dropout.
        src_dist_embedded = self.relu1(src_dist_embedded)  # Apply ReLU activation.
   
        # Embed the env sequence for environmental encoding.
        src_env_embedded = self.embedding_layer(env, is_src=False, src_tensor=src, type_tensor=type)
        src_env_embedded = self.dropout2(src_env_embedded)  # Apply dropout.
        src_env_embedded = self.relu2(src_env_embedded)  # Apply ReLU activation.

        # Process spatial, temporal, and environmental embeddings through respective encoders.
        spatial_input_embedded = self.spatial_encoder_1(src_dist_embedded, mask=src_mask)
        temporal_input_embedded = self.temporal_encoder_1(src_temporal_embedded, mask=src_mask)
        environmental_input_embedded = self.environmental_encoder_1(src_env_embedded, mask=src_mask)


        # Fuse spatial, temporal, and environmental features.
        fusion_feat = torch.cat((temporal_input_embedded, environmental_input_embedded, spatial_input_embedded), dim=2)
        fusion_feat = self.fusion_layer(fusion_feat)  # Combine features using the fusion layer.

        # Process the fused features through secondary encoders.
        spatial_output = self.spatial_encoder_2(fusion_feat)
        environmental_output = self.environmental_encoder_2(spatial_output)
        temporal_output = self.temporal_encoder_2(environmental_output)
        ...
        output1 = self.decoder1(temporal_output)
        output2 = self.decoder2(temporal_output)
        output3 = self.decoder3(temporal_output)
        output4 = self.decoder4(temporal_output)
        output5 = self.decoder5(temporal_output)
        output6 = self.decoder6(temporal_output)

        # Concatenate the outputs from all decoders.
        output = torch.cat([output1, output2, output3, output4, output5, output6], dim=1)
        output = output.reshape(src.shape[0], self.tgt_len, self.output_size)
